...
import com.github.lamkenn.kafka.tutorial1.exception.ConsumerException;
...
public class BlockingQueueConsumer<K, V> implements ConsumerRebalanceListener {
    ...
    Map<TopicPartition, Long> endOffsetsPartitionMap;
    ...
    public void start() {
        synchronized (lock) {
            ...
            relay = new ConsumerRecordRelay<>(this, consumer);
    ...
    private void createProcessor(TopicPartition partition) {
        Long endOffset = endOffsetsPartitionMap.get(partition);
        Processor<K, V> processor = new Processor<>(partition, relay, action, queueSize, endOffset);
